...
class AM_SimCLR(object):
    ...
    def info_nce_loss(self, feature_map, temperature):
        ...
        similarity_matrix = torch.matmul(feature_map, feature_map.T)
        ...
        labels = labels[~mask].view(labels.shape[0], -1)
        similarity_matrix = similarity_matrix[~mask].view(similarity_matrix.shape[0], -1)
        temperature = temperature * self.args.bias + self.args.temperature

        # assert similarity_matrix.shape == labels.shape

        # select and combine multiple positives
        positives = similarity_matrix[labels.bool()].view(labels.shape[0], -1)
        # temperature_p = temperature[labels.bool()].view(temperature.shape[0], -1) + self.args.temperature
        logits_positives = positives / temperature

        # select only the negatives the negatives
        negatives = similarity_matrix[~labels.bool()].view(similarity_matrix.shape[0], -1)
        # temperature_n = temperature[~labels.bool()].view(temperature.shape[0], -1) + self.args.temperature
        logits_negitives = negatives / temperature

        logits = torch.cat([logits_positives, logits_negitives], dim=1)
        labels = torch.zeros(logits.shape[0], dtype=torch.long).to(self.args.device)
